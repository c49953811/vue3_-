<template>
  <div class="goods-comment">
    ...
    <!-- 分页组件 pageSize currentPage  -->
    <XtxPagination
      v-if="total"
      :total="total"
      :pageSize="reqParams.pageSize"
      :currentPage="reqParams.page"
      @current-page="changePageFn"
    />
  </div>
</template>
<script>
...
export default {
  ...
  setup() {
    ...
    findGoodsCommentInfo(goods.value.id).then((data) => {
      // 数据有问题，需要在data.result追加 有图tag 全部评价tag
    ...
    // 激活tag
    const currentTagIdex = ref(0)
    const changeTag = (i) => {
      currentTagIdex.value = i
      // 修改筛选条件
